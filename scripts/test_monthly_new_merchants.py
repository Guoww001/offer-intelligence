...
def main():
    ...
    optional_values = offer_db._monthly_new_merchant_values(
        {"reportMonth": "2026-08", "merchantName": "Name only"},
        updated_by="admin",
    )
    ...
    try:
        ...
        created_row = {
            ...
            "createdBy": "admin",
            "updatedBy": "admin",
            "createdAt": dt.datetime(2026, 8, 6, 4, 0, 0),
            "updatedAt": dt.datetime(2026, 8, 6, 4, 0, 0),
        }
        ...
        updated = offer_db.upsert_monthly_new_merchant(updated_payload, updated_by="editor")
        assert_equal(updated["action"], "updated", "update action")
        assert_equal(updated["record"]["isPriority"], False, "cleared priority")
        ...
        duplicate = offer_db.upsert_monthly_new_merchant(
            {"reportMonth": "2026-08", "merchantName": "August Merchant"},
            updated_by="admin",
        )
        ...
        assert_equal(deleted["record"]["isPriority"], True, "deleted priority")
        ...
        assert_equal(listed["priorityCount"], 1, "priority merchant count")
        ...
        assert_equal(listed["source"], "cnpscy_oi_monthly_new_merchants", "database source")

        print("Monthly new merchant manual persistence checks passed")
    finally:
