...
assert(m6ModernPages.get("agent")?.status === "dual", "Chat Agent 视觉等价完成前必须保持 dual 状态");
...
assert(parityGate[1].includes("__OI_MODERN_CHATBOT_AGENT_PARITY__ === true"), "视觉等价完成前必须显式 true 才启用 Modern 对照");
assert(!parityGate[1].includes("__OI_MODERN_CHATBOT_AGENT_PARITY__ !== false"), "视觉等价完成前不得默认启用 Modern");
